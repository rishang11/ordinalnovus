...
function processMetaprotocolParams(
  ...
  console.debug(`Processing metaprotocols parameter: ${value}`); // Log the input tagsValue for debugging

  // Check if the values is using the 'or' operator
  if (value.includes("cbrc-20")) {
    // finalQuery.find.parsed_metaprotocol = value;
    finalQuery.find["$and"] = [
      { parsed_metaprotocol: value },
      { parsed_metaprotocol: "transfer" },
    ];
    finalQuery.find["$or"] = [{ valid: true }, { valid: { $exists: false } }];
  }
  // If no operator is found, treat it as a single tag
  else {
    finalQuery.find.parsed_metaprotocol = value;
...
function processWhereParam(
  ...
  if (key === "content" || key === "search") {
    ...
    if (model.collection.name === "cbrctokens")
      finalQuery.find.$text = { $search: `\"${value}\"` };
    else if (model.collection.name === "txes")
      finalQuery.find["$and"] = [{ token: value, parsed: true }];
    else
      finalQuery.find["$and"] = [
        {
...
/**
 * Parses a query string and builds the MongoDB query object.
 * Supports conditions like _or:token>false|tags>!text
 * @param queryString The query string to be parsed.
 * @returns The parsed MongoDB query object.
 */
function parseQueryString(
  ...
  if (logicalOperator.startsWith("_or")) {
    finalQuery.find.$or = conditions
      .split("|")
      .map((condition) => parseCondition(condition));
  } else if (logicalOperator.startsWith("_and")) {
    finalQuery.find.$and = conditions
      .split("|")
      .map((condition) => parseCondition(condition));
  } else {
...
/**
 * Parses an individual condition and converts it to a MongoDB query condition.
 * @param condition The condition to be parsed (e.g., token>false or tags>!text).
 * @returns The parsed MongoDB condition object.
 */
function parseCondition(condition: string): any {
  const conditionObject: any = {};

  // Split by the `>` operator to handle negation and comparison
  const [field, value] = condition.split(">");

  if (value.startsWith("!")) {
    conditionObject[field] = { $ne: value.substring(1) };
  } else if (value === "true" || value === "false") {
    conditionObject[field] = value === "true";
  } else {
...
export default function convertParams(
  ...
  Object.keys(params).forEach((key) => {
    if (key.includes("_sort")) {
      ...
      parseQueryString(finalQuery, key, params["_or"]);
    } else if (key === "tags") {
